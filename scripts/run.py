...
def set_seed(config):
    ...
    np.random.seed(seed)
    # If you're using random library anywhere
    import random
    random.seed(seed)
    # For some torch operations
...
def main(config: "DictConfig"):
    ...
    logging.basicConfig(
        ...
        datefmt='%Y-%m-%d %H:%M:%S',  
        handlers=[
            logging.FileHandler(os.path.join(config.save_path, "terminal.log")),  
    ...
    if config.agent_name == "autoui":
        agent = AutoUIAgent(device=device, accelerator=accelerator, 
                            ...
                            use_causal=config.use_causal, causal_save_path=os.path.join(config.save_path, "causal"),
                            )
