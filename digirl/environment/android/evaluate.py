...
def call_gemini(client, system_msg, prompt, image_list, image_path):
    ...
    for j in range(2):
        try:
            ...
            # wait for waiting_time second before retrying. waiting_time is a random number between 3 and 5
            waiting_time = random.uniform(2.0, 10.0)
            time.sleep(waiting_time)
            logging.info(f"Retrying... {j+1} times")
            if 'suspended' in str(e):
                response_text = "Error: google.api_core.exceptions.ResourceExhausted: 429 Resource has been suspended"
            else:
                response_text = "Error: google.api_core.exceptions.ResourceExhausted: 429 Resource has been exhausted"
