...
from digirl.algorithms.causal_digirl import CausalDigiRLTrainer
...
def remove_empty_instances(all_trajectories):
    new_trajectories = []
...
def offpolicy_train_loop(env,\
    ...
    if train_algorithm == "digirl":
        trainer = CausalDigiRLTrainer(agent=agent,\
    ...
    if use_causal:
        causal_replay_buffer = ReplayBuffer(batch_size= batch_size, capacity=100000)
        ...
        if os.path.exists(os.path.join(save_path, 'trainer_offline.pt')):
            ...
            if offline_data_path is not None and train_mode != "online":
                ...
                if train_algorithm == "filteredbc":
                    ...
                    for i in tqdm(range(offline_actor_iterations), disable=not accelerator.is_main_process):
                        info.update(trainer.update_policy(filtered_buffer, filtered_validation_buffer, is_online=False))
                        if use_wandb and accelerator.is_main_process:
                            wandb.log(info)
    ...
    # pretrain causal model
    if use_causal: 
        res = {}
        for _ in range(causal_pretrain):
            res = trainer.update_causal(causal_replay_buffer, validation_buffer)
        print(">>> Pretraining of Causal Model is Done !!")
        print(res)
        
        del causal_replay_buffer
    ...
    for i in range(resume_iter, train_iterations):
        ...
        trajectories = remove_empty_instances(trajectories)
        trajectories = framestack(trajectories)
        if accelerator.is_main_process:
            info = {"iteration": i,\
                    "rollout.mean": np.mean([d[0]["trajectory_reward"] if len(d) > 0 else 0 for d in trajectories]),\
                    "rollout.max": np.max([d[0]["trajectory_reward"] if len(d) > 0 else -1 for d in trajectories]),\
                    "rollout.min": np.min([d[0]["trajectory_reward"] if len(d) > 0 else -1 for d in trajectories]),\
                    "walltime": time.time()}
        ...
        colorful_print(f">>> length of replay_buffer: {len(replay_buffer)}", fg='green')
        colorful_print(f">>> length of filtered_buffer: {len(filtered_buffer)}", fg='green')
        ...
        if 'filtered' in train_algorithm:
            ...
            if use_causal:
                info.update(trainer.update_causal(replay_buffer, validation_buffer))
            info.update(trainer.update_trajectory_critic(train_trajectories, val_trajectories))
