...
from digirl.coso import parse_action
...
class ActionTypeDataset(torch.utils.data.Dataset):
...
class CausalDigiRLTrainer():
    def __init__(self, agent,\
                 accelerator,\
                    tokenizer,\
                    critic_lr: float = 1e-3,\
                    lm_lr: float = 1e-5,\
                    causal_lr: float = 1e-5,\
        ...
        if use_causal:
            self.causal_model_optimizer = torch.optim.Adam(agent.causal_model.parameters(), lr = causal_lr)
        ...
        self.causal_epochs = causal_epochs
    ...
    def prepare(self):
        ...
        if self.use_causal:
            self.causal_model_optimizer = self.accelerator.prepare(self.causal_model_optimizer)
    ...
    def actor_loss(self, observation, action, image_features, next_observation, next_image_features, mc_return, pi_action, advantage, reward,
                   validation=False, is_online=True, **kwargs):
        mc_return = torch.Tensor(mc_return).to(self.accelerator.unwrap_model(self.agent.model).device, dtype = self.accelerator.unwrap_model(self.agent.model).dtype).flatten()
        reward = torch.Tensor(reward).to(self.accelerator.unwrap_model(self.agent.model).device, dtype = self.accelerator.unwrap_model(self.agent.model).dtype).flatten()
        with torch.no_grad():
            v1, v2 = self.agent.critic(observation, image_features, action, detach_model=False)
        ...
        log_prob, entropy, causal_entropy = self.agent.get_log_prob(observation, image_features, action, return_entropy=True)
        ...
        if self.use_entropy and self.use_causal and is_online:
            total_loss = pg_loss - self.entropy_coeff * causal_entropy.mean()
        elif self.use_entropy and not self.use_causal and is_online:
            total_loss = pg_loss - self.entropy_coeff * entropy.mean()
        else:
            total_loss = pg_loss
        value_loss = torch.zeros_like(total_loss)
        if not validation:
            self.accelerator.backward(total_loss+value_loss)
        ...
        info =  {"pg.loss": pg_loss.detach().cpu().item(),
                ...
                "entropy.mean": entropy.mean().detach().cpu().item(),
                "entropy.max": torch.max(entropy).detach().cpu().item(),
                "entropy.min": torch.min(entropy).detach().cpu().item(),
                "entropy.std": torch.std(entropy).detach().cpu().item(),
                "causal_entropy.mean": causal_entropy.mean().detach().cpu().item() if self.use_causal else 0,
                "causal_entropy.max": torch.max(causal_entropy).detach().cpu().item() if self.use_causal else 0,
                "causal_entropy.min": torch.min(causal_entropy).detach().cpu().item() if self.use_causal else 0,
                "causal_entropy.std": torch.std(causal_entropy).detach().cpu().item() if self.use_causal else 0,
                }
        if validation:
            validation_info = {}
            for k,v in info.items():
    ...
    def update_trajectory_critic(self, trajectories, validation_trajectories = None):
        print(">>>Updating Trajectory Critic")
        info = {}
        info_list = []
        batch_size = 8
        with torch.autograd.set_detect_anomaly(True):
    ...
    def update_policy(self, replay_buffer, validation_buffer = None, no_update_actor=False, is_online=True):
        self.step += 1
        info = {}
        info_list = []
        ...
        if not no_update_actor:
            ...
            for _ in tqdm(range(self.actor_epochs), disable= not self.accelerator.is_main_process):
                ...
                for batch in dataloader:
                    pi_action = None
                    advantages = None
                    info_list.append(self.actor_loss(**batch, pi_action=pi_action, advantage=advantages, is_online=is_online))
                self.accelerator.clip_grad_norm_(self.agent.parameters(), self.max_grad_norm)
        ...
        if validation_buffer is not None:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader, disable=True):
                    info_list.append(self.actor_loss(validation=True, pi_action=None, advantage=None, is_online=False, **batch))
            info.update(dict_mean(info_list))
            return info
        return info

    def update_causal(self, replay_buffer, validation_buffer = None):
        # print(">>>Updating Causal Model")
        assert self.use_causal == True, "Causal model is not enabled"
        self.agent.causal_model.train()
        train_num, val_num = 1024 , 128
        # num_epochs = 1
        info_list = []
        info = {}
        for _ in tqdm(range(self.causal_epochs), disable= not self.accelerator.is_main_process):
            train_data = [replay_buffer.sample(1)['action'].item() for _ in range(train_num)]
            
            train_raw_actions, train_labels, train_valids = parse_action(train_data)
            train_raw_actions = [action for action, v in zip(train_raw_actions, train_valids) if v==1]
            train_labels = [label for label, v in zip(train_labels, train_valids) if v==1]

            train_encodings = self.agent.tokenizer(train_raw_actions, return_tensors='pt', padding=True, max_length=512, truncation = True)
            train_loader = DataLoader(ActionTypeDataset(train_encodings, train_labels), batch_size=512, shuffle=True)

            train_loader = self.accelerator.prepare(train_loader)
            # self.causal_model_optimizer.zero_grad()
            # for _ in tqdm(range(num_epochs), disable= not self.accelerator.is_main_process):
            self.causal_model_optimizer.zero_grad()
            epoch_loss = 0
            correct = 0
            total = 0
            ...
            for batch in train_loader:
                input_ids = batch['input_ids'].to(self.agent.device)
                attention_mask = batch['attention_mask'].to(self.agent.device)
                labels = batch['labels'].to(self.agent.device)
                outputs = self.agent.causal_model(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
            ...
            info_list.append({"causal.loss": avg_epoch_loss, "causal.acc": train_accuracy})
            self.accelerator.clip_grad_norm_(self.agent.causal_model.parameters(), self.max_grad_norm)
            self.causal_model_optimizer.step()

        info.update(dict_mean(info_list))
                
        # val 
        if validation_buffer is not None:
            self.agent.causal_model.eval()
            info_list = []
            val_data = [validation_buffer.sample(1)['action'].item() for _ in range(val_num)]
            val_raw_actions, val_labels, val_valids = parse_action(val_data)
            val_raw_actions = [action for action, v in zip(val_raw_actions, val_valids) if v==1]
            val_labels = [label for label, v in zip(val_labels, val_valids) if v==1]
            if len(val_raw_actions) == 0:
                return info
            
            val_encodings = self.agent.tokenizer(val_raw_actions, return_tensors='pt', padding=True, max_length=512, truncation = True)
            val_loader = DataLoader(ActionTypeDataset(val_encodings, val_labels), batch_size=256, shuffle=False)
            ...
            info_list.append({"validation.causal.loss": avg_val_loss, "validation.causal.acc": val_accuracy})
            info.update(dict_mean(info_list))
        return info
